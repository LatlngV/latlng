...
class WebsiteSurvey(http.Controller):
    ...
    @http.route(['/survey/start/<model("survey.survey"):survey>',
                 '/survey/start/<model("survey.survey"):survey>/<string:token>/<model("employee.passive.survey"):employee>'],
                type='http', auth='public', website=True)
    def start_survey(self, survey, employee, token=None, **post):
        ...
        if token and token == "phantom":
            ...
            user_input = UserInput.create(
                {'survey_id': survey.id, 'test_entry': True, 'passive_name': employee.passive_survey.name_related})
